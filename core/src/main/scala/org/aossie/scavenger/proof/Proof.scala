...
import collection.mutable.{Stack, HashMap => MMap, HashSet => MSet}
...
class Proof[P <: ProofNode[Clause,P]](val root: P, val leftRight: Boolean = true, val permutation: IndexedSeq[P] = IndexedSeq[P]())
extends Iterable[P]
{
...
object Proof {
  implicit def apply[P <: ProofNode[Clause,P]](root: P) = new Proof(root)
}
